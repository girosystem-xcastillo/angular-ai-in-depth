...
export const MOCK_REPLIES: string[] = [
  "That's a great question about Angular! Signals provide a reactive primitive that's synchronous and always hold a current value. They automatically track their dependencies, so any computed signal or effect that reads a signal will re-run when that signal changes. This makes state management much simpler compared to manual subscription management with RxJS.",
  "Angular's component architecture promotes reusability and separation of concerns. Each component encapsulates its template, styles, and logic, making applications easier to test and maintain. The `@Component` decorator defines the component's metadata, including its selector, template, and styles.",
  'The Angular CLI is a powerful tool that helps you scaffold, build, test, and deploy Angular applications. Commands like `ng generate`, `ng build`, and `ng serve` are part of your daily workflow. The CLI also handles configuration, so you can focus on writing application code rather than build tooling.',
  "Angular uses TypeScript by default, providing type safety, interfaces, and modern JavaScript features. TypeScript helps catch errors at compile time and improves IDE support with better autocomplete and refactoring tools. The strict mode in TypeScript, enabled by default in Angular projects, helps write more robust code.",
  "Dependency injection in Angular is a design pattern that allows you to declare dependencies as constructor parameters or via the `inject()` function, and Angular's DI system provides the appropriate instances. This promotes loose coupling, testability, and modularity in your application.",
];
